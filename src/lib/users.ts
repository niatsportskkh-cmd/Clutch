...
export async function signupGate(raw: { phone?: unknown; collegeId?: unknown }): Promise<GateResult> {
  ...
  if (!phone.success) return { ok: false, message: 'Enter a valid mobile number' }
  ...
  if (!student) {
    return { ok: false, message: 'That college ID and mobile number are not on the student list together. Check both, or ask the organisers to add you.' }
  }
  if (await users().findOne({ collegeId: collegeId.data })) {
    return { ok: false, message: 'An account already exists for this college ID. Log in instead, or reset the password.' }
  }
