...
export function CollegeForm({ locations }: { locations: string[] }) {
  ...
  return (
    <form action={action} className="flex flex-col gap-5">
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="College" name="name" required maxLength={60} autoComplete="off" defaultValue={kept?.name} placeholder="KKH" />
        {/* a datalist, not a select: the first college in a new city has to be able to name that city */}
        <Field label="Location" name="location" required maxLength={60} autoComplete="off" list="known-locations" defaultValue={kept?.location} placeholder="Hyderabad" />
        <datalist id="known-locations">{locations.map(l => <option key={l} value={l} />)}</datalist>
      </div>
      <div className="flex flex-wrap items-center gap-4">
        <Button type="submit" disabled={pending} className="px-8">{pending ? 'Saving' : 'Add college'}</Button>
        <p aria-live="polite" className={state && !state.ok ? 'text-danger' : 'text-volt'}>{state ? (state.ok ? state.message : state.error) : ''}</p>
      </div>
      <p className="text-sm text-muted">A college already on the list has its location updated, not duplicated.</p>
    </form>
  )
}
